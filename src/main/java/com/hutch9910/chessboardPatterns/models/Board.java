...
public class Board {
    ...
    public EnumSet<Team>[][] getAttacks() {
        return attacks;
    }
    ...
    public void setAttacks() {
        this.attacks = (EnumSet<Team>[][]) new EnumSet[this.boardSide][this.boardSide];
    ...
    public void addAttackedPositions(Piece piece) {
        ...
        for (Point attackedPosition : attackedPositions) {
            ...
            if (x < attacks[0].length && x >= 0 && y < attacks.length && y >= 0) { // check if the position under attack is inside the board
